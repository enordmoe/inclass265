...
This implies November 5th will be the first Thursday of November. 
...
Thanksgiving will be on November 28 in 2030.
...
(date <- date+months(10))
wday(date, label = TRUE, abbr = FALSE)
wday(date+6, label = TRUE, abbr = FALSE) # Add 6 to get from Friday to Thursday
(date <- date + days(6) + weeks(3))
```



# Exercise 2: The **lakers** data set  
...
**Answer**:  Most on Tuesday, least on Monday and Saturday.
...
lakers %>% 
  distinct(date) %>% 
  mutate(month_of_game = month(date, label = TRUE, abbr = 
                                 FALSE)) %>%
#  ggplot(aes(x = fct_inorder(factor(month_of_game)))) +
   ggplot(aes(x = fct_inorder(factor(month_of_game)))) +
  geom_bar() + 
  labs(x = "Month of Game")
```

The played the most games (15) in both January and March.


# 2. Distribution of Plays Througought the Game  
