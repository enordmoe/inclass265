...
1. The web site [NY Times Best Sellers: Hardcover Fiction](http://www.nytimes.com/books/best-sellers/hardcover-fiction) contains a list of best-selling fiction books. Scrape the names, authors, publishers and weeks on the list (`parse_number()`?) of these top books. Use SelectorGadget to obtain the appropriate css selector. 

2. The web site [BikeRaceInfo](http://www.bikeraceinfo.com/tdf/tdfstats.html) has a table with data on past winners of the Tour de France. Use the methods shown above using Wikipedia to create a cleaned-up data frame of this data for the subtable for races between 1903 and 1959. This will require several steps and likely involve splitting columns after the basic extraction. (If you prefer, you could try using the selector gadget but the table structure of this page suggests using the table approach shown above for Wikipedia.)
